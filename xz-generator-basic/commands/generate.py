...
from .utils import get_root_path, get_basic_path, find_file_by_name
...
def generate():
    author = Prompt.ask('输入作者')
    loop = typer.confirm('是否循环')
    output_text = Prompt.ask('输出文本')
    ...
    # 2.替换模板内容
    env = Environment(loader=FileSystemLoader(os.path.join(get_basic_path(), 'templates')))
    template = env.get_template('MainTemplate.java-tpl')
    output = template.render({'author': author, 'loop': loop, 'output_text': output_text})
